                                  ...
                                  const double pos_increment)
{
  ...
  {
    RCLCPP_WARN_STREAM(LOGGER,"Negative position/orientation intcrement: " << pos_increment << "/" << orient_increment);
    rtn.clear();
    return rtn;
  }
  ...
  RCLCPP_DEBUG_STREAM(LOGGER,"Estimated number of samples: " << est_num_samples << ", reserving space");
  ...
  RCLCPP_DEBUG_STREAM(LOGGER,"Uniform sampling of frame, utilizing orientation increment: "
...
double distance(const std::vector<double> &j1, const std::vector<double> &j2)
{
  ...
  {
    RCLCPP_WARN_STREAM(LOGGER,"Unequal size vectors, returning negative distance");
    return -1;
  }
...
void CartTrajectoryPt::getCartesianPoses(const RobotModel &model, EigenSTL::vector_Isometry3d &poses) const
{
  ...
  {
    RCLCPP_ERROR(LOGGER,"Failed for find ANY cartesian poses");
  }

  if (poses.empty())
  {
    RCLCPP_WARN(LOGGER,"Failed for find VALID cartesian poses, returning");
  }
  else
  {
    RCLCPP_DEBUG_STREAM(LOGGER,"Get cartesian poses, sampled: " << all_poses.size() << ", with " << poses.size()
                                           ...
                                           std::vector<double> &joint_pose) const
{
  ...
  {
    RCLCPP_ERROR_STREAM(LOGGER,"FK failed for seed pose for closest joint pose");
    return false;
  }
  ...
  for (std::size_t i = 0; i < vals.size(); i++)
  {
    ...
    if (std::abs(upper - lower) > EQUALITY_TOLERANCE)
    {
      ...
      if (std::minmax({ lower, v, upper }) != bounds)
      {
        ...
        RCLCPP_DEBUG(LOGGER,"Cartesian nominal [%lu] exceeded bounds: [val: %f, lower: %f, upper: %f]", i, v, lower, upper);
      }
    }
    else
    {
      ...
      {
        solve_ik = true;
        RCLCPP_DEBUG(LOGGER,"Cartesian nominals [%lu] differ: [val: %f, lower: %f, upper: %f]", i, v, lower, upper);
  ...
  if (solve_ik)
  {
    ...
    {
      RCLCPP_WARN_STREAM(LOGGER,"Ik failed on closest pose");

      std::vector<std::vector<double> > joint_poses;
      getJointPoses(model, joint_poses);
      if (joint_poses.size() > 0)
      {
        RCLCPP_WARN_STREAM(LOGGER,"Closest cartesian pose not found, returning closest to seed joint pose");
      ...
      {
        RCLCPP_ERROR_STREAM(LOGGER,"getClosestJointPose failed, no valid joint poses for this point");
        return false;
      }
...
void CartTrajectoryPt::getJointPoses(const RobotModel &model, std::vector<std::vector<double> > &joint_poses) const
{
  ...
  {
    RCLCPP_WARN(LOGGER,"Failed for find ANY joint poses, returning");
  }
  else
  {
    RCLCPP_DEBUG_STREAM(LOGGER,"Get joint poses, sampled: " << poses.size() << ", with " << joint_poses.size()
                                                  << " valid(returned) poses");
  }
}
